...
async def autopilot():
    ...
    if not channel:
        ...
        try:
            r = await ultroid_bot(
                CreateChannelRequest(
                    title="My Ragdoll Logs",
                    about="My Ragdoll Log Group\n\n Join @TeamRagdoll",
                    megagroup=True,
                ),
            )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo, _ = await download_file(
            "https://graph.org/file/0fa2242dfa9067ed57406.jpg", "channelphoto.jpg"
        )
...
async def customize():
    ...
    try:
        ...
        file = random.choice(
            [
                "https://graph.org/file/2dbf49b52878e6f7de035.jpg",
                "https://graph.org/file/19dab4080f5cb19bd141c.jpg",
        ...
        await ultroid_bot.send_message(
            "botfather",
            f"✨ Powerful Ragdoll Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @TeamRagdoll ✨",
        )
        await asyncio.sleep(2)
            ...
            async for x in ultroid_bot.iter_messages(
                ...
                if not os.path.exists(plugin):
                    ...
                    try:
                        ...
                        LOGS.info(f"Ragdoll - PLUGIN_CHANNEL - ERROR - {plugin}")
                        LOGS.exception(e)
                        os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """🎇 **Thanks for Deploying Ragdoll Userbot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://graph.org/file/51cea1fb4847952f49e6a.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**Ragdoll has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(ultroid_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @TeamRagdoll\n➖➖➖➖➖➖➖➖➖➖"
